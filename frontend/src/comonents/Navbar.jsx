...
function Navbar() {
  ...
  const handleLogout = async (e) => {
    ...
    try {
      ...
      localStorage.removeItem("jwt"); // delete local storage token
  ...
  return (
    <>
      <nav className="shadow-lg px-4 py-2">
        ...
        {/* Mobile Navbar */}
        {show && (
          <div className="bg-blue-200 md:hidden">
            <ul className="flex flex-col items-center justify-center space-y-3 text-xl">
              ...
              <div className="w-full mt-4 flex flex-col space-y-2">
                ...
                {!isAuthenticated ? (
                  <Link to="/login" onClick={() => setShow(!show)} className="bg-red-600 text-white font-semibold hover:bg-red-800 duration-300 px-4 py-2 rounded w-full">
                    LOGIN
                  </Link>
                ) : (
                  <button onClick={handleLogout} className="bg-red-600 text-white font-semibold hover:bg-red-800 duration-300 px-4 py-2 rounded w-full">
                    LOGOUT
                  </button>
                )}
              </div> */}
...
//     e.preventDefault();
//     try {
//       // Replace the hardcoded URL with the dynamic BACKEND_URL
